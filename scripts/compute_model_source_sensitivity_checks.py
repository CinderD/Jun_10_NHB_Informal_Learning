...
def write_section24_table(rows: list[dict[str, str]]) -> None:
    ...
    with open(path, "w") as f:
        ...
        f.write(
            "\\caption{\\todayrevise{\\textbf{Metadata sensitivity for Section~\\ref{subsec:2.4} support-form models.} "
            ...
            "This metadata check complements, but does not replace, the descriptive percentage-point support-form contrasts in Fig.~\\ref{fig:support_form_supply}.}}\\label{tab:section24_support_form_model_source_sensitivity}\n"
        )
        f.write("\\setlength{\\tabcolsep}{5pt}\n\\renewcommand{\\arraystretch}{1.10}\n")
        f.write("\\resizebox{0.82\\textwidth}{!}{%\n")
        f.write("\\begin{tabular}{lcc}\n\\toprule\n")
        f.write("Support form & Dataset FE & \\todayrevise{Metadata FE} \\\\\n\\midrule\n")
        ...
        f.write(f"\\todayrevise{{Metadata fixed effects}} & -- & {model_fe} \\\\\n")
        f.write("\\bottomrule\n\\end{tabular}%\n}\n\\end{table*}\n")
