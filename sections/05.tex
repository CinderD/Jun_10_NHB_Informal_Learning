...
\subsection*{B.2\quad Cross-dataset and metadata regression checks}
...
Integrated adjacent-turn regressions were used as robustness checks rather than as the primary narrative claim. These models predicted whether the next user turn was constructive after combining user-state features, assistant-scaffolding features and adjacent-turn context. Because metadata differ by corpus, we report both within-setting models and pooled specifications. The within-setting models show the dataset-by-task pattern directly, with available assistant-model or service-source fixed effects added separately inside each setting (Supplementary Table~\ref{tab:setting_level_adjacent_models}). The pooled specification includes dataset fixed effects; a sensitivity replaces these with exact WildChat model labels, LMSYS model names parsed from conversation identifiers and ShareChat assistant or source families parsed from conversation identifiers. This is interpreted as metadata adjustment rather than as a causal comparison among model versions. Metadata heterogeneity was detectable, but it did not absorb the scaffolding signal. Adding broad scaffolded support to user/context/model controls improved fit (LR $\chi^2_1=509.7$, $p<.001$). Because M1--M6 are support forms nested within scaffolded support rather than independent covariates, we interpret the decomposed models as asking which forms carry additional signal among scaffolded turns, not as a test of scaffolded support after removing its own components. Adding M1--M6 descriptors within scaffolded support improved fit (LR $\chi^2_6=829.8$, $p<.001$), and the full scaffolding block remained strongly informative after controls (LR $\chi^2_7=1339.6$, $p<.001$; Supplementary Table~\ref{tab:integrated_scaffolding_block_tests}). Adding prior-state $\times$ support-form interactions also improved fit (pooled metadata fixed effects: LR $\chi^2_{18}=297.3$, $p<.001$; Supplementary Table~\ref{tab:prior_state_support_form_interactions}). Fully decomposed coefficients clarify the source of this form-level signal: prior user state is the largest predictor of immediate constructive follow-up, and M4 explaining is the most stable positive support-form coefficient (Supplementary Table~\ref{tab:integrated_regression}).
