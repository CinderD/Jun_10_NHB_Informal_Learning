...
Together, these results show that constructive engagement is ecological. It is amplified by intentional user framing, varies across task ecologies and concentrates in sustained exchanges. A simple conversation-level logistic check showed the same directional organization after including user framing, task ecology, conversation-length bucket and dataset fixed effects (Supplementary Table~\ref{tab:constructive_context_logit}).
...
\begin{figure*}[t]
  ...
  \includegraphics[width=\linewidth]{figures/fig_support_association_wild_lmsys_with_ci.pdf}
    ...
    Numbers indicate scaffolded-minus-reference differences, except panels b and c, which report model estimates. Error bars in panels b and c show model-based 95\% confidence intervals; Poisson overdispersion sensitivity is reported in Methods and the Appendix.
    }
