...
\latestrevise{Where assistant-model or source-family metadata were recoverable, supplementary checks summarized the patterns below across these observational metadata and tested whether key associations were preserved after metadata adjustment\footnote{Assistant-model metadata are available in WildChat and LMSYS Chat; service-source metadata are available in ShareChat.} (Supplementary Tables~\ref{tab:section21_model_source_breakdown}--\ref{tab:prior_state_support_form_interactions}).}
