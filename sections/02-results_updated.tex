...
\latestrevise{Where assistant-model or source-family metadata were recoverable, supplementary analyses assessed whether the patterns reported below varied across dataset, model or source families and whether the key associations remained after accounting for these differences\footnote{Assistant-model metadata are available in WildChat and LMSYS Chat; service-source metadata are available in ShareChat.} (Supplementary Tables~\ref{tab:section21_model_source_breakdown}--\ref{tab:prior_state_support_form_interactions}).}
