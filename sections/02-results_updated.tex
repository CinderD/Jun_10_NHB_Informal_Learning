...
\revise{Where recoverable assistant-model or source-family metadata were available, supplementary checks preserved the substantive patterns reported below (Supplementary Tables~\ref{tab:section21_model_source_breakdown}, \ref{tab:section22_model_source_sensitivity}, \ref{tab:section23_model_source_sensitivity} and \ref{tab:section24_support_form_model_source_sensitivity}).}
...
We then asked whether the form of support shaped this adjacent-turn coupling once prior user state and adjacent-turn context were modelled jointly. We fitted integrated logistic regressions predicting whether the next user turn was constructive, combining prior user state, assistant-scaffolding descriptors, task and framing variables, turn position and \revise{model or source fixed effects}. The scaffolding block improved fit beyond user state and context (full scaffolding block LR $\chi^2_7=1339.6$, $p<.001$; Supplementary Table~\ref{tab:integrated_scaffolding_block_tests}), and prior-state $\times$ support-form interactions improved fit further (\revise{pooled metadata fixed effects}: LR $\chi^2_{18}=297.3$, $p<.001$; Supplementary Table~\ref{tab:prior_state_support_form_interactions}). \revise{The corresponding pooled coefficient estimates and setting-level fits are reported in Supplementary Tables~\ref{tab:integrated_regression} and \ref{tab:setting_level_adjacent_models}.} These tests show that the immediate support--engagement association was not reducible to the user's preceding state alone; it depended jointly on what the user had just made visible and how the assistant supported the next step.
