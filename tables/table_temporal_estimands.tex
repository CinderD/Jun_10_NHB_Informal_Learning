...
\tiny
\centering
\caption{\textbf{Temporal estimands used to characterize local support--engagement coupling.} Main temporal analyses use adjacent turn pairs and short-range event windows. Coarser before--after summaries are exploratory boundary checks because scaffolded turns are selected by task difficulty, user motivation and conversation trajectory.}
...
Short-range event sequence & Event windows mark scaffolded support in the current assistant response or the preceding one to three assistant responses; a lagged logistic model enters current and lagged scaffolded-response indicators together. & Section~2.5 and Supplementary Table~\ref{tab:event_sequence_lagged_scaffolding} & Tests whether the temporal signal extends over recent responses without treating the whole conversation as cumulative exposure. \\
