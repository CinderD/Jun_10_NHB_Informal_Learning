...
& \multicolumn{1}{c}{\revise{Emotional expression}} \\
\cmidrule(lr){2-4}
\cmidrule(lr){5-5}
\cmidrule(lr){6-6}
\cmidrule(lr){7-8}
...
& \makecell{\revise{Emotional}\\\revise{expression}\\\revise{(\%)}} \\
\midrule
...
\footnotesize
\textit{Note.} User framing is reported as the percentage of conversations with explicit learning-oriented framing. \revise{Cognitive and constructive engagement and emotional expression are percentages of user turns;} scaffolded support is computed over assistant turns. \revise{Assistant turns refer to AI/LLM responses, so user-side and assistant-side percentages use different role-specific denominators.}
\end{minipage}
\end{table*}
